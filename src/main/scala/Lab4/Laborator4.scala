...
object Ex4 {
    ...
    val sequence1= Apply(player1Bet1,player2Bet,player1Bet2,
    (bet11:Boolean, bet2:Boolean,bet12:Boolean)=>
        if(!bet11 && bet2 && bet12) 1.0
        else 0.0
    )

    val algSequence1= VariableElimination(sequence1)
    algSequence1.start()
    algSequence1.stop()
    println("The probability for the firt sequence:" + algSequence1.mean(sequence1))


      val sequence2= Apply(player1Bet1,player2Bet,player1Bet2,
    (bet11:Boolean, bet2:Boolean,bet12:Boolean)=>
        if(!bet11 && bet2 && bet12) 1.0
        else 0.0
    )

    val algSequence2= VariableElimination(sequence1)
    algSequece2.start()
    algSequence2.stop()
    println("The probability for the second sequence:" + algSequence2.mean(sequence2))




      val sequence3= Apply(player1Bet1,player2Bet,player1Bet2,
          (bet11:Boolean, bet2:Boolean,bet12:Boolean)=>
            if(bet11 && bet2 && bet12) 1.0
         else 0.0
         )

     val algSequence3= VariableElimination(sequence1)
    algSequence3.start()
    algSequence3.stop()
    println("The probability for the third sequence:" + algSequence3.mean(sequence3))
}